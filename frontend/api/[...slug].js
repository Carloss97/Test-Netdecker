...
module.exports = async function (req, res) {
  try {
    ...
    // Build potential handler paths under repo-root _api_handlers
    const baseHandlers = path.join(__dirname, '..', '..', '_api_handlers');
    const byParts = path.join(baseHandlers, ...parts) + '.js';
    const byPartsPhp = path.join(baseHandlers, ...parts) + '.php.js';
    const byIndex = path.join(baseHandlers, ...parts, 'index.js');

    if (fs.existsSync(byParts)) {
      const h = require(byParts);
      return await h(req, res);
    }
    if (fs.existsSync(byPartsPhp)) {
      const h = require(byPartsPhp);
      return await h(req, res);
    }
    if (fs.existsSync(byIndex)) {
      const h = require(byIndex);
      return await h(req, res);
    }
