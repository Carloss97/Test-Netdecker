import { pickDb, ensureSchema, getTableColumns } from '../../_shared/d1.js';
...
export async function onRequest(context) {
  ...
  try {
    ...
    const existing = await getTableColumns(db, 'card');
    let covered = 0;
    let uncovered = 0;
    let percent = 0;
    let byTcg = [];
    const hasPriceMarket = existing.includes('priceMarket');
    if (hasPriceMarket) {
      const coveredRes = await db.prepare('SELECT COUNT(*) AS cnt FROM card WHERE priceMarket IS NOT NULL;').all();
      covered = (Array.isArray(coveredRes?.results) ? coveredRes.results[0]?.cnt : (Array.isArray(coveredRes) ? coveredRes[0]?.cnt : 0)) || 0;
      ...
      byTcg = byRows.map((r) => ({ tcg: r.tcg, tcgDisplayName: r.tcg, totalCards: r.totalCards || 0, coveredCards: r.coveredCards || 0, uncoveredCards: Math.max(0, (r.totalCards || 0) - (r.coveredCards || 0)), coveragePercent: ((r.totalCards || 0) === 0 ? 0 : ((r.coveredCards || 0) / (r.totalCards || 0)) * 100) }));
    } else {
      // fallback when priceMarket missing: report totals and zero coverage
      covered = 0;
      uncovered = total;
      percent = 0;
