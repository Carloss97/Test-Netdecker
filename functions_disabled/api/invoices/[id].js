...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!inv) return new Response(JSON.stringify({ success: false, error: 'Invoice not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    return new Response(JSON.stringify({ success: true, invoice: inv }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err) {
