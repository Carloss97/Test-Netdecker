...
const DATA_DIR = path.resolve(process.cwd(), process.argv[2] || 'testRA5');
...
const OUTPUT_PATH = path.join(DATA_DIR, 'YGO_mapped.xlsx');
...
function getSigla(rarityText) {
  ...
  // Collector remains Collector (COL) for the general script
...
// Build output rows starting with template headers and map all rows
(async () => {
