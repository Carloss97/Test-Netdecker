...
export async function onRequest(context) {
  ...
  try {
    ...
    let listingSelect = listingCols && listingCols.length ? listingCols : 'l.id AS listingId, l.cardId, l.editionCode, l.referencePrice, l.marginMultiplier, l.finalPrice, l.quantity, l.status, l.lastSyncedAt';
    ...
    const sql = `SELECT ${listingSelect}, ${selectCard} FROM listing l JOIN card c ON l.cardId = c.id WHERE c.externalId = ? OR c.id = ? OR l.cardId = ? ORDER BY quantity DESC, finalPrice ASC`;
