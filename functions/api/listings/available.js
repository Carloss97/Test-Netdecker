...
export async function onRequest(context) {
  ...
  try {
    ...
    const tcgId = url.searchParams.get('tcgId') || url.searchParams.get('tcg');
    const editionId = url.searchParams.get('editionId') || url.searchParams.get('edition');
    ...
    let sql = `SELECT ${listingCols}, ${cardCols} FROM listing l LEFT JOIN card c ON c.id = l.cardId WHERE l.quantity > 0 AND l.status IN ('active','manual')`;
    const binds = [];
    if (tcgId) {
      sql += ' AND c.tcg = ?'; binds.push(tcgId);
    }
    if (editionId) {
      sql += ' AND (l.editionCode = ? OR c.editionCode = ?)'; binds.push(editionId, editionId);
    }

    sql += ' ORDER BY l.finalPrice ASC LIMIT 200';
